 ...
 *
 * KORJAUKSET (2026-05-08):
 * - politicalEvents haetaan suoraan fetchPoliticalEvents()-funktiolla
 *   paikalliseen stateen, koska DashboardContext ei tarjoa sita kenttaa.
 *   Tama oli paasyysiita etta Sapina-valilehti nayttaa tyhjaa sivua.
 * - Lisatty ErrorBoundary, joka estaa koko komponentin kaatumisen
 *   yksittaisen datalahteen virheesta.
 * - Lisatty null-/undefined-tarkistukset kaikkiin forEach/map-kutsuihin.
 * - setManualTolppa: "" -> null korjattu vastaamaan kirjastofunktion signatuuria.
 */
...
// ---------------------------------------------------------------------------
// ErrorBoundary — estaa koko komponentin kaatumisen yksittaisesta virheesta
// ---------------------------------------------------------------------------
...
> {
  ...
  render() {
    if (this.state.hasError) {
      return (
        <div className="rounded-xl bg-destructive/10 border border-destructive/30 p-5 flex items-start gap-3">
          <AlertTriangle className="h-5 w-5 text-destructive shrink-0 mt-0.5" />
          <div>
            <p className="font-black text-sm text-destructive uppercase tracking-wider">
              Aikajana ei latautunut
            </p>
            ...
            <button
              ...
              className="mt-2 text-xs font-black text-primary uppercase tracking-wider"
            >
              Yritä uudelleen
            </button>
          </div>
      ...
      >
        ...
        <div className="border-t border-border mt-1 pt-1">
          <button
            onClick={(e) => {
              e.stopPropagation();
              // null = poista override kokonaan (tyhja string ei riita)
              setManualTolppa(item.id, null);
              setOpen(false);
            }}
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <p className="font-black text-lg text-foreground truncate leading-tight">
          {item.title}
        </p>
        ...
        <div className="flex items-center gap-1 mt-1">
          ...
          {canEditTolppa && <TolppaEditor item={item} />}
        </div>
          ...
          {item.tag && (
            <span
              className={`inline-block text-[10px] font-black uppercase tracking-wider px-2 py-0.5 rounded ${
                item.tag.includes("LOPPUUNMYYTY") ||
                item.tag.includes("KORKEA") ||
                item.tag.includes("TÄYNNÄ")
                  ? "bg-destructive/20 text-destructive"
...
// ---------------------------------------------------------------------------
// EventsTimeline — sisainen komponentti (ilman ErrorBoundarya)
// ---------------------------------------------------------------------------

interface EventsTimelineProps {
  /** Avaa detail-paneelin klikatulle itemille */
...
const EventsTimelineInner = ({ onSelect, onAddEvent }: EventsTimelineProps) => {
  ...
  // -------------------------------------------------------------------------
  // KORJAUS: DashboardContext ei tarjoa politicalEvents-kenttaa.
  // Haetaan se paikallisesti suoraan fetchPoliticalEvents()-funktiolla.
  // -------------------------------------------------------------------------
  ...
  useEffect(() => {
    ...
    const load = () => {
      fetchPoliticalEvents()
        .then((data) => {
          if (!cancelled) setPoliticalEvents(data ?? []);
        })
        .catch((err) => {
          console.warn("[EventsTimeline] fetchPoliticalEvents epäonnistui:", err);
          if (!cancelled) setPoliticalEvents([]);
        });
    };

    load();
    const interval = setInterval(load, 10 * 60 * 1000); // 10 min paivitys
  ...
  // Triggeri uudelleenrenderille kun kayttaja muuttaa manuaalista tolppa-overridea
  ...
  const allItems: TimelineItem[] = useMemo(() => {
    ...
    (state.sportsEvents ?? [])
      .filter((s) =>
        !isLowTaxiDemandEvent(`${s.homeTeam} ${s.awayTeam}`, s.venue),
      )
      .forEach((s) => items.push(sportsToTimelineItem(s)));
    // politicalEvents on nyt turvallinen paikallinen state — ei koskaan undefined
    (politicalEvents ?? []).forEach((p) =>
      items.push(politicalToTimelineItem(p)),
    );

    // Manuaaliset tolppa-overridet
    const overridden = items.map((it) => {
      const m = getManualTolppa(it.id);
      if (m === undefined) return it;       // ei overridea
      if (m === null) return { ...it, tolppa: undefined }; // kayttaja poisti
      return { ...it, tolppa: m };
    });
  ...
  // -------------------------------------------------------------------------
  // Suodata aika-ikkunan mukaan ja ryhmita kategorioihin
  // -------------------------------------------------------------------------
    ...
    const sortByWeight = (a: TimelineItem, b: TimelineItem) => {
      if (a.tolppaKmFromUser != null && b.tolppaKmFromUser != null) {
        const distBoost = (km: number, weight: number) => {
          if (km <= 5) return 60 - km * 8;
          if (km <= 10) return 20 - (km - 5) * 4;
  ...
  const visibleToday = isExpanded
    ? todayItems
    : todayItems.slice(0, HARD_LIMIT_PER_TAB);
  const visibleUpcoming = isUpcomingOpen
    ? isExpanded
      ? upcomingItems
      : upcomingItems.slice(0, HARD_LIMIT_PER_TAB)
    : [];
  const hiddenToday = todayItems.length - visibleToday.length;
  const hiddenUpcoming = isUpcomingOpen
    ? upcomingItems.length - visibleUpcoming.length
    : 0;
          ...
          <div className="flex flex-col gap-3">
            ...
            {upcomingItems.length > 0 && (
              <div className="flex flex-col gap-2">
                ...
                >
                  <span>
                    Tulevat tapahtumat ({upcomingItems.length})
                  </span>
                  <ChevronDown
            ...
            {hiddenCount > 0 && (
              ...
              >
                Näytä kaikki ({hiddenCount} lisää)
              </button>
            )}
...
// ---------------------------------------------------------------------------
// Exportattu komponentti — kaeritty ErrorBoundarylla
// ---------------------------------------------------------------------------

const EventsTimeline = (props: EventsTimelineProps) => (
  <TimelineErrorBoundary>
    <EventsTimelineInner {...props} />
  </TimelineErrorBoundary>
);
